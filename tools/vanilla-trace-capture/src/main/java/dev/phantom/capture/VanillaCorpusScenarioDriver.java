...
public final class VanillaCorpusScenarioDriver {
    ...
    private static final int RESET_SETTLE = 3;
    ...
    public static String phaseLabel() { return "corpus:" + (INSTANCE == null ? "unknown" : INSTANCE.currentScenario()); }
    private String currentScenario() { return index >= 0 && index < SCENARIOS.length ? SCENARIOS[index] : requestedScenario; }
    ...
    public void tick(MinecraftClient client) {
        ...
        if (!requested.equals("all") && position(requested) < 0) throw new IllegalArgumentException("Unknown corpus scenario: " + requested);
        if (!requested.equals(requestedScenario)) {
            requestedScenario = requested; index = requested.equals("all") ? 0 : position(requested);
            elapsed = 0; prepared = false; resetting = false; resetTicks = 0; release(client.options);
        }
        if (index < 0 || index >= SCENARIOS.length) { release(client.options); return; }
        String scenario = SCENARIOS[index];
        if (!prepared) {
            if (!resetting) {
                prepareScenario(client, scenario); resetting = true; resetTicks = 0; release(client.options);
            } else if (++resetTicks >= RESET_SETTLE) {
                prepared = true; resetting = false; elapsed = 0; release(client.options);
            }
            return;
        }
        configureInput(client.options, scenario, elapsed);
        triggerScenarioEvent(client, scenario, elapsed);
        elapsed++;
        if (elapsed < TICKS) return;
        release(client.options);
        System.out.println("[Phase5] CORPUS COMPLETE scenario=" + scenario);
        if (!requestedScenario.equals("all")) { client.scheduleStop(); return; }
        index++;
        if (index >= SCENARIOS.length) { System.out.println("[Phase5] CORPUS COMPLETE all"); client.scheduleStop(); return; }
        prepared = false; resetting = false; resetTicks = 0;
    }
    ...
    private void prepareScenario(MinecraftClient client, String id) {
        ...
        server.executeSync(() -> {
            ...
            cmd(server, "gamemode survival @a");
            cmd(server, "effect clear @a");
            ...
            cmd(server, "time set day");
            cmd(server, "weather clear");
            cmd(server, "fill -8 63 0 8 63 110 minecraft:stone");
            cmd(server, "fill -8 64 0 8 80 110 air");
            setupWorld(server, world, id);
            setupEffects(server, id);
            applyAttributeFixture(player, id);
            if (isAirborneScenario(id)) {
            ...
            if (id.equals("glide")) player.equipStack(EquipmentSlot.CHEST, new ItemStack(Items.ELYTRA));
        });
        System.out.println("[Phase5] CORPUS START scenario=" + id);
    }

    private static boolean isAirborneScenario(String id) {
        return id.equals("controlled-fall") || id.equals("landing") || id.equals("slow-falling") || id.equals("levitation") || id.equals("knockback-air") || id.equals("glide");
    }

    private static void applyAttributeFixture(ServerPlayerEntity player, String id) {
        EntityAttributeInstance movement = player.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED);
    ...
    private static void setupWorld(IntegratedServer server, ServerWorld world, String id) {
        ...
        switch (id) {
            ...
            case "step-up", "step-sprint-jump" -> cmd(m, s, "fill -2 64 25 2 64 25 minecraft:stone");
            case "partial-collision" -> cmd(m, s, "fill 1 64 25 1 66 29 minecraft:stone");
            ...
            case "ladder", "ladder-sprint", "climb-jump" -> cmd(m, s, "fill -1 64 30 1 68 30 minecraft:ladder[facing=north]");
    ...
    private static void setupEffects(IntegratedServer server, String id) {
        switch (id) {
            case "speed-effect" -> cmd(server, "effect give @a minecraft:speed 600 1 true");
            case "slowness-effect" -> cmd(server, "effect give @a minecraft:slowness 600 1 true");
            case "jump-boost" -> cmd(server, "effect give @a minecraft:jump_boost 600 2 true");
            case "slow-falling" -> cmd(server, "effect give @a minecraft:slow_falling 600 0 true");
            case "levitation" -> cmd(server, "effect give @a minecraft:levitation 600 1 true");
            case "lava" -> cmd(server, "effect give @a minecraft:fire_resistance 600 0 true");
            default -> { }
        }
    }

    private static void triggerScenarioEvent(MinecraftClient client, String id, int t) {
        IntegratedServer server = client.getServer();
        if (server == null || client.player == null) return;
        server.executeSync(() -> {
            ServerPlayerEntity player = server.getPlayerManager().getPlayer(client.player.getUuid());
            if (player == null) return;
            if ((id.equals("knockback-ground") || id.equals("knockback-air") || id.equals("correction-after-knockback")) && t == 20) player.setVelocity(0.8D, 0.32D, 0.25D);
        ...
        if (id.equals("glide") && t == 4 && client.player.getEquippedStack(EquipmentSlot.CHEST).isOf(Items.ELYTRA) && client.player.checkGliding()) client.player.startGliding();
    }
