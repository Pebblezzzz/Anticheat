...
public final class Phase6Reachability {
  ...
  public record Context(long simulationTick,Player player,Simulation.Environment environment,Simulation.Attributes attributes,
                        ...
                        EntityCollisions entityCollisions,Set<UncertainDimension> uncertainty,Simulation.MovementCapabilities capabilities) implements Serializable {
    public Context(long tick,Player player,Simulation.Environment env,Simulation.Attributes attributes,MovementEffects effects,Pose pose,MovementEnvironment movementEnvironment,boolean sleeping){this(tick,player,env,attributes,effects,pose,movementEnvironment,sleeping,EntityCollisions.of(List.of()),Set.of(),Simulation.MovementCapabilities.NONE);}
    public Context(long tick,Player player,Simulation.Environment env,Simulation.Attributes attributes,MovementEffects effects,Pose pose,MovementEnvironment movementEnvironment,boolean sleeping,EntityCollisions entityCollisions){this(tick,player,env,attributes,effects,pose,movementEnvironment,sleeping,entityCollisions,Set.of(),Simulation.MovementCapabilities.NONE);}
    public Context(long tick,Player player,Simulation.Environment env,Simulation.Attributes attributes,MovementEffects effects,Pose pose,MovementEnvironment movementEnvironment,boolean sleeping,EntityCollisions entityCollisions,Set<UncertainDimension> uncertainty){this(tick,player,env,attributes,effects,pose,movementEnvironment,sleeping,entityCollisions,uncertainty,Simulation.MovementCapabilities.NONE);}
    public Context{if(simulationTick<0)throw new IllegalArgumentException("simulationTick must be non-negative");Objects.requireNonNull(player);Objects.requireNonNull(environment);Objects.requireNonNull(attributes);Objects.requireNonNull(effects);Objects.requireNonNull(pose);Objects.requireNonNull(movementEnvironment);Objects.requireNonNull(entityCollisions);uncertainty=Set.copyOf(uncertainty);Objects.requireNonNull(capabilities);}
    public Context withTick(long tick){return new Context(tick,player,environment,attributes,effects,pose,movementEnvironment,sleeping,entityCollisions,uncertainty,capabilities);}
    public Context withUncertainty(UncertainDimension... dimensions){EnumSet<UncertainDimension> u=EnumSet.noneOf(UncertainDimension.class);u.addAll(uncertainty);u.addAll(List.of(dimensions));return new Context(simulationTick,player,environment,attributes,effects,pose,movementEnvironment,sleeping,entityCollisions,u,capabilities);}
  }
  ...
  public record TeleportCorrection(int id,Maths.Vec3 position,Maths.Vec3 velocity,Pose pose,boolean awaitingConfirmation) implements ExternalTransition{public TeleportCorrection{if(id<0)throw new IllegalArgumentException("teleport id must be non-negative");Objects.requireNonNull(position);Objects.requireNonNull(velocity);Objects.requireNonNull(pose);}}
  ...
  public SearchResult search(Context start,List<InputConstraint> inputs,LongFunction<List<WorldBranch>> worlds,LongFunction<List<ExternalTransition>> externalTransitions,int maximumCandidates){
    ...
    if(start.capabilities().authorizedFlight())return new SearchResult(Verdict.POSSIBLE,Set.of(new Candidate(0,start,new Provenance(0,-1,start.simulationTick(),"AUTHORIZED_FLIGHT","ROOT","None",List.of("server-authorized flight state"),1,List.of()))),0,1,0,0,0,0,List.of("server-authorized flight state is outside walking reachability"));
      ...
      for(Candidate parent:current.values())for(WorldBranch branch:branches){Maths.Aabb pb=Maths.Aabb.playerAt(parent.context().player().position(),parent.context().pose());if(!branch.world().fullyKnown(new dev.phantom.ac.geometry.BlockBox(pb.minX(),pb.minY(),pb.minZ(),pb.maxX(),pb.maxY(),pb.maxZ()))){uncertainTransitions++;continue;}
        ...
        for(AdvancedInput input:allowed){
          ...
          Vanilla12111RichPhysics.Context rc=new Vanilla12111RichPhysics.Context(tick,pre.player(),input,branch.world(),
              simulationEnvironment,pre.attributes(),pre.effects(),pre.pose(),env,pre.sleeping(),pre.entityCollisions(),pre.capabilities());
          ...
          Context after=new Context(tick+1,stepped.state(),environmentFor(nextEnvironment),pre.attributes(),pre.effects(),nextPose,
              nextEnvironment,pre.sleeping(),pre.entityCollisions(),pre.uncertainty(),pre.capabilities());Candidate existing=next.get(after);
