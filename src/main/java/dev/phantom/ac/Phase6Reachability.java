...
public final class Phase6Reachability {
  ...
  public record Context(long simulationTick,Player player,Simulation.Environment environment,Simulation.Attributes attributes,
    ...
    public Context(long tick,Player player,Simulation.Environment env,Simulation.Attributes attributes,MovementEffects effects,Pose pose,MovementEnvironment movementEnvironment,boolean sleeping){
      this(tick,player,env,attributes,effects,pose,movementEnvironment,sleeping,Set.of());
    }
    ...
    public Context withUncertainty(UncertainDimension... dimensions){EnumSet<UncertainDimension> u=EnumSet.copyOf(uncertainty.isEmpty()?EnumSet.noneOf(UncertainDimension.class):uncertainty);u.addAll(List.of(dimensions));return new Context(simulationTick,player,environment,attributes,effects,pose,movementEnvironment,sleeping,u);}
  }

  /** Partial client input; empty dimensions retain all declared discrete possibilities. */
  ...
  public SearchResult search(Context start,List<InputConstraint> inputs,LongFunction<List<WorldBranch>> worlds,LongFunction<List<ExternalTransition>> externalTransitions,int maximumCandidates){
    ...
    for(int offset=0;offset<inputs.size();offset++){
      ...
      for(Candidate parent:current.values())for(WorldBranch branch:branches){
        Maths.Aabb pb=Maths.Aabb.playerAt(parent.context().player().position(),parent.context().pose());
        if(!branch.world().fullyKnown(new dev.phantom.ac.geometry.BlockBox(pb.minX(),pb.minY(),pb.minZ(),pb.maxX(),pb.maxY(),pb.maxZ()))){uncertainTransitions++;continue;}
        for(ExternalTransition event:external){Context pre=applyExternal(parent.context(),event,tick);if(pre.player().uncertain()){uncertainTransitions++;continue;}
          for(AdvancedInput input:allowed){MovementEnvironment env=adjustEnvironment(pre.movementEnvironment(),pre.player(),input);Vanilla12111RichPhysics.Context rc=new Vanilla12111RichPhysics.Context(tick,pre.player(),input,branch.world(),pre.environment(),pre.attributes(),pre.effects(),pre.pose(),env,pre.sleeping());Vanilla12111RichPhysics.StepResult stepped=physics.step(rc);if(stepped.state().uncertain()){uncertainTransitions++;continue;}
            Context after=new Context(tick+1,stepped.state(),pre.environment(),pre.attributes(),pre.effects(),stepped.pose(),adjustEnvironment(env,stepped.state(),input),pre.sleeping());
            Candidate existing=next.get(after);
      ...
      peak=Math.max(peak,next.size());current=next;
      if(nonExhaustive>0)return new SearchResult(Verdict.UNCERTAIN,Set.of(),offset+1,peak,merged,nonExhaustive,uncertainTransitions,provenanceMerges,List.of("world hypothesis envelope is not exhaustive at tick "+tick));
    }
