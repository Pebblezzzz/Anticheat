...
public final class Timeline {
  ...
  public static final class Codec {
    ...
    private static Packet readPacket(DataInputStream in)throws IOException{return switch(in.readUnsignedByte()){
      ...
      case 14->{String gamemode=readString(in);Simulation.Attributes attributes=readAttributes(in);int effectCount=readCount(in,"effect");Map<String,Integer> effects=new HashMap<>();for(int i=0;i<effectCount;i++){String id=readString(in);if(effects.put(id,in.readInt())!=null)throw new IllegalArgumentException("duplicate player effect");}Phase5Mechanics.Pose pose=ordinal(Phase5Mechanics.Pose.values(),in.readUnsignedByte(),"pose");Phase5Mechanics.MovementEnvironment environment=readMovementEnvironment(in);boolean sleeping=in.readBoolean();int entityCount=readCount(in,"entity box");List<dev.phantom.ac.world.EntityCollisions.EntityBox> entities=new ArrayList<>();for(int i=0;i<entityCount;i++)entities.add(new dev.phantom.ac.world.EntityCollisions.EntityBox(in.readInt(),readBlockBox(in)));boolean canFly=in.readBoolean(),flying=in.readBoolean();yield new PlayerContext(gamemode,attributes,effects,pose,environment,sleeping,entities,new Simulation.MovementCapabilities(canFly,flying));}
