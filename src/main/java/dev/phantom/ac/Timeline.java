...
public final class Timeline {
  ...
  public static final class Codec {
    ...
    private static Packet readPacket(DataInputStream in)throws IOException{return switch(in.readUnsignedByte()){
      ...
      case 14->{String gamemode=readString(in);Simulation.Attributes attributes=readAttributes(in);int effectCount=readCount(in,"effect");Map<String,Integer> effects=new HashMap<>();for(int i=0;i<effectCount;i++){String id=readString(in);if(effects.put(id,in.readInt())!=null)throw new IllegalArgumentException("duplicate player effect");}Pose pose=ordinal(Phase5Mechanics.Pose.values(),in.readUnsignedByte(),"pose");Phase5Mechanics.MovementEnvironment environment=readMovementEnvironment(in);Maths.Vec3 serverPosition=readVec(in);Maths.Vec3 serverVelocity=readVec(in);boolean canFly=in.readBoolean();boolean flying=in.readBoolean();boolean sleeping=in.readBoolean();int entityCount=readCount(in,"entity box");List<dev.phantom.ac.world.EntityCollisions.EntityBox> entities=new ArrayList<>();for(int i=0;i<entityCount;i++)entities.add(new dev.phantom.ac.world.EntityCollisions.EntityBox(in.readInt(),readBlockBox(in)));yield new PlayerContext(gamemode,attributes,effects,pose,environment,serverPosition,serverVelocity,canFly,flying,sleeping,entities);}
