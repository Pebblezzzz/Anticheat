...
public final class Timeline {
  ...
  private static final short FORMAT_VERSION=10;
  ...
  /** Fixed binary replay format. v10 persists lossless block properties plus client-visible entity state; v9 remains readable. */
  public static final class Codec {
    ...
    private static Event readEvent(DataInputStream in,int version)throws IOException{long tick=in.readLong(),seq=in.readLong(),nanos=in.readLong();EnumSet<PacketFlag> flags=readFlags(in.readInt());CaptureProvenance provenance=version>=2?readProvenance(in,version):null;Packet packet=readPacket(in,version);return new Event(tick,new NormalizedPacket(seq,nanos,packet,flags,provenance));}
    private static void writeProvenance(DataOutputStream out,CaptureProvenance p)throws IOException{writeString(out,p.sourceId());writeString(out,p.direction());writeString(out,p.packetType());writeNullableLong(out,p.authoritativeServerTick());writeNullableLong(out,p.authoritativeClientTick());}
    private static CaptureProvenance readProvenance(DataInputStream in,int version)throws IOException{String source=readString(in);String direction=readString(in);String packetType=readString(in);Long serverTick=readNullableLong(in);Long clientTick=version>=8?readNullableLong(in):null;return new CaptureProvenance(source,direction,packetType,serverTick,clientTick);}
    ...
    private static void writePacket(DataOutputStream out,Packet p)throws IOException{
      ...
      else if(p instanceof EntitySpawn v){out.writeByte(20);out.writeInt(v.entityId());writeBlockBox(out,v.box());}
      else if(p instanceof EntityMove v){out.writeByte(21);out.writeInt(v.entityId());writeBlockBox(out,v.box());}
      else if(p instanceof EntityDespawn v){out.writeByte(22);out.writeInt(v.entityId());}
      else if(p instanceof PlayerContext v){out.writeByte(14);writeString(out,v.gamemode());writeAttributes(out,v.attributes());out.writeInt(v.effects().size());List<Map.Entry<String,Integer>> effects=new ArrayList<>(v.effects().entrySet());effects.sort(Comparator.comparing(Map.Entry::getKey));for(var e:effects){writeString(out,e.getKey());out.writeInt(e.getValue());}out.writeByte(v.pose().ordinal());writeMovementEnvironment(out,v.movementEnvironment());writeVec(out,v.serverPosition());writeVec(out,v.serverVelocity());out.writeBoolean(v.canFly());out.writeBoolean(v.flying());out.writeBoolean(v.sleeping());out.writeInt(v.entityBoxes().size());for(var entity:v.entityBoxes()){out.writeInt(entity.entityId());writeBlockBox(out,entity.box());}}
      else if(p instanceof FlightToggle v){out.writeByte(18);out.writeBoolean(v.flying());out.writeBoolean(v.cancelled());}
    ...
    private static Packet readPacket(DataInputStream in,int version)throws IOException{return switch(in.readUnsignedByte()){
      ...
      case 14->{String gamemode=readString(in);Simulation.Attributes attributes=readAttributes(in);int effectCount=readCount(in,"effect");Map<String,Integer> effects=new HashMap<>();for(int i=0;i<effectCount;i++){String id=readString(in);if(effects.put(id,in.readInt())!=null)throw new IllegalArgumentException("duplicate player effect");}Phase5Mechanics.Pose pose=ordinal(Phase5Mechanics.Pose.values(),in.readUnsignedByte(),"pose");Phase5Mechanics.MovementEnvironment environment=readMovementEnvironment(in);Maths.Vec3 serverPosition=readVec(in);Maths.Vec3 serverVelocity=readVec(in);boolean canFly=in.readBoolean();boolean flying=in.readBoolean();boolean sleeping=in.readBoolean();int entityCount=readCount(in,"entity box");List<dev.phantom.ac.world.EntityCollisions.EntityBox> entities=new ArrayList<>();for(int i=0;i<entityCount;i++)entities.add(new dev.phantom.ac.world.EntityCollisions.EntityBox(in.readInt(),readBlockBox(in)));yield new PlayerContext(gamemode,attributes,effects,pose,environment,serverPosition,serverVelocity,canFly,flying,sleeping,entities);}
