...
public final class Vanilla12111RichPhysics {
    ...
    public StepResult step(Context context){
        ...
        if(s.gamemode().equals("creative")||s.gamemode().equals("spectator"))return new StepResult(context.simulationTick(),new Player(s.position(),Vec3.ZERO,s.yaw(),s.pitch(),false,s.gamemode(),s.effects(),s.awaitingTeleport(),false),false,false,false,false,false,false,false,"non-physical gamemode");
        if(!s.gamemode().equals("survival")&&!s.gamemode().equals("adventure"))return uncertain(context,"unsupported gamemode movement model");

        Phase5Mechanics.MovementEnvironment env=context.movementEnvironment();Phase5Mechanics.Pose pose=Phase5Mechanics.nextPose(context.pose(),env,context.sleeping());Aabb start=Aabb.playerAt(s.position(),pose);var startEntities=context.entityCollisions().boxesIn(new dev.phantom.ac.geometry.BlockBox(start.minX(),start.minY(),start.minZ(),start.maxX(),start.maxY(),start.maxZ()));if(!startEntities.isDefinite())return uncertain(context,"entity collision history is incomplete");
        ...
        RichWorldCollision.Result collision=RichWorldCollision.resolve(context.world(),start,velocity,s.onGround()&&!fluid&&!climbing&&!gliding?STEP_HEIGHT:0,context.entityCollisions());if(collision.uncertain())return uncertain(context,collision.diagnostic());Vec3 displacement=collision.displacement();boolean grounded=collision.collidedY()&&velocity.y()<=0;
        double horizontalFactor;if(env.fluid()==Phase5Mechanics.Fluid.WATER)horizontalFactor=env.fluidSpeedMultiplier()*WATER_DRAG;else if(env.fluid()==Phase5Mechanics.Fluid.LAVA)horizontalFactor=env.fluidSpeedMultiplier()*LAVA_DRAG;else if(climbing)horizontalFactor=s.onGround()?GROUND_FRICTION:AIR_HORIZONTAL_FRICTION;else if(gliding)horizontalFactor=AIR_DRAG;else horizontalFactor=s.onGround()?GROUND_FRICTION:AIR_HORIZONTAL_FRICTION;
        ...
        double groundedVerticalVelocity=context.effects().levitation()?context.effects().levitationVelocity():climbing?velocity.y():(env.fluid()==Phase5Mechanics.Fluid.WATER||env.fluid()==Phase5Mechanics.Fluid.LAVA)?velocity.y()*env.fluidDrag()-gravity:-gravity*AIR_VERTICAL_DRAG*context.effects().fallGravityMultiplier();double nextY=grounded&&!context.effects().levitation()&&!climbing?groundedVerticalVelocity:postTickVerticalVelocity;Vec3 nextVelocity=new Vec3(collision.collidedX()?0:velocity.x()*horizontalFactor,nextY,collision.collidedZ()?0:velocity.z()*horizontalFactor);
        Player next=new Player(s.position().add(displacement),nextVelocity,s.yaw(),s.pitch(),grounded,s.gamemode(),s.effects(),s.awaitingTeleport(),false);return new StepResult(context.simulationTick(),next,collision.collidedX()||collision.collidedY()||collision.collidedZ(),collision.stepAttempted(),collision.stepSucceeded(),collision.collidedX(),collision.collidedY(),collision.collidedZ(),false,collision.diagnostic());
    }

    private StepResult uncertain(Context context,String message){Player s=context.state();return new StepResult(context.simulationTick(),new Player(s.position(),s.velocity(),s.yaw(),s.pitch(),s.onGround(),s.gamemode(),s.effects(),s.awaitingTeleport(),true),false,false,false,false,false,false,false,message);}
