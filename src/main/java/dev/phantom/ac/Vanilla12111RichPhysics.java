...
public final class Vanilla12111RichPhysics {
    ...
    public StepResult step(Context context){
        ...
        if(context.capabilities().flying()&&!context.capabilities().canFly())return uncertain(context,"server reported flying without an authorized flight capability");
        if(context.capabilities().authorizedFlight())return new StepResult(context.simulationTick(),s,false,false,false,false,false,false,false,"server-authorized flight bypasses walking physics until flight-specific simulation is validated");
