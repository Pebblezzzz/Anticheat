...
/** Phase 5 movement engine consuming the exact coverage-aware 1.21.11 world snapshot. */
public final class Vanilla12111RichPhysics {
    ...
    private static final double DIAGONAL_ACCEL=0.1,WATER_DRAG=0.9,LAVA_DRAG=0.5,CLIMB_MAX_DOWN=0.15,CLIMB_MAX_UP=0.15,GLIDE_GRAVITY=0.035;

    public StepResult step(Context context) {
        Objects.requireNonNull(context,"context"); Player s=context.state();
        ...
        if(s.gamemode().equals("creative")||s.gamemode().equals("spectator")) return new StepResult(context.simulationTick(),new Player(s.position(),Vec3.ZERO,s.yaw(),s.pitch(),false,s.gamemode(),s.effects(),s.awaitingTeleport(),false),false,false,false,false,false,false,"non-physical gamemode");
        ...
        double speed=context.attributes().value()*context.effects().speedMultiplier()*(context.input().sprint()?1.3:1.0)*(context.input().sneak()?0.3:1.0);
        boolean fluid=env.fluid()!=Phase5Mechanics.Fluid.NONE,climbing=env.climbable(),gliding=env.gliding();
        double inputMagnitude=Math.hypot(context.input().forward(),context.input().strafe()),inputScale=inputMagnitude>1.0?1.0/Math.sqrt(2.0):1.0;
        double inputAcceleration;
        if(fluid) inputAcceleration=AIR_ACCEL; else if(s.onGround()&&inputMagnitude>1.0) inputAcceleration=DIAGONAL_ACCEL*context.effects().speedMultiplier()*(context.input().sprint()?1.3:1.0)*(context.input().sneak()?0.3:1.0); else inputAcceleration=s.onGround()?WALK_ACCEL*speed:AIR_ACCEL;
        if(gliding) inputAcceleration=AIR_ACCEL;
        Vec3 acceleration=new Vec3(inputScale*(context.input().strafe()*inputAcceleration*Math.cos(radians)-context.input().forward()*inputAcceleration*Math.sin(radians)),0,inputScale*(context.input().forward()*inputAcceleration*Math.cos(radians)+context.input().strafe()*inputAcceleration*Math.sin(radians)));
        ...
        Vec3 displacement=collision.displacement(); boolean grounded=collision.collidedY()&&velocity.y()<=0;

        double horizontalFactor;
        if(env.fluid()==Phase5Mechanics.Fluid.WATER)horizontalFactor=env.fluidSpeedMultiplier()*WATER_DRAG;else if(env.fluid()==Phase5Mechanics.Fluid.LAVA)horizontalFactor=env.fluidSpeedMultiplier()*LAVA_DRAG;else if(climbing)horizontalFactor=s.onGround()?GROUND_FRICTION:AIR_HORIZONTAL_FRICTION;else if(gliding)horizontalFactor=AIR_DRAG;else horizontalFactor=s.onGround()?GROUND_FRICTION:AIR_HORIZONTAL_FRICTION;
        double gravity=GRAVITY*env.gravityMultiplier(); double postTickVerticalVelocity;
        if(context.effects().levitation())postTickVerticalVelocity=context.effects().levitationVelocity();else if(climbing)postTickVerticalVelocity=velocity.y();else if(env.fluid()==Phase5Mechanics.Fluid.WATER||env.fluid()==Phase5Mechanics.Fluid.LAVA)postTickVerticalVelocity=velocity.y()*env.fluidDrag()-gravity;else if(gliding)postTickVerticalVelocity=velocity.y()-GLIDE_GRAVITY;else postTickVerticalVelocity=(jumped?velocity.y()-gravity*context.effects().fallGravityMultiplier():velocity.y()*AIR_VERTICAL_DRAG-gravity*context.effects().fallGravityMultiplier()*AIR_VERTICAL_DRAG);
        double groundedVerticalVelocity=context.effects().levitation()?context.effects().levitationVelocity():climbing?velocity.y():(env.fluid()==Phase5Mechanics.Fluid.WATER||env.fluid()==Phase5Mechanics.Fluid.LAVA)?velocity.y()*env.fluidDrag()-gravity:-gravity*AIR_VERTICAL_DRAG*context.effects().fallGravityMultiplier();
    ...
    private StepResult uncertain(Context context,String message){Player s=context.state();return new StepResult(context.simulationTick(),new Player(s.position(),s.velocity(),s.yaw(),s.pitch(),s.onGround(),s.gamemode(),s.effects(),s.awaitingTeleport(),true),false,false,false,false,false,false,message);}

    public record Context(long simulationTick,Player state,Simulation.AdvancedInput input,WorldSnapshot world,Simulation.Environment environment,Simulation.Attributes attributes,Phase5Mechanics.MovementEffects effects,Phase5Mechanics.Pose pose,Phase5Mechanics.MovementEnvironment movementEnvironment,boolean sleeping) implements Serializable {
    ...
    public record StepResult(long simulationTick,Player state,boolean collided,boolean stepAttempted,boolean stepSucceeded,boolean collisionX,boolean collisionY,boolean collisionZ,String diagnostic) implements Serializable {public StepResult{Objects.requireNonNull(state);Objects.requireNonNull(diagnostic);}}
}
