...
public final class State {
  ...
  public record Player(Vec3 position,Vec3 velocity,float yaw,float pitch,boolean onGround,String gamemode,Map<String,Integer> effects,OptionalInt awaitingTeleport,boolean uncertain,
      Optional<Simulation.AdvancedInput> input,Simulation.Attributes attributes,Phase5Mechanics.Pose pose,Environment environment,TickRange clientTickRange,Provenance provenance,Set<UncertaintyReason> uncertaintyReasons,int jumpDelay) implements Serializable {
    public Player {
      ...
      if(uncertain&&uncertaintyReasons.isEmpty())uncertaintyReasons=Set.of(UncertaintyReason.EXPLICIT_UNCERTAINTY);if(jumpDelay<0||jumpDelay>10)throw new IllegalArgumentException("jumpDelay must be between 0 and 10");
    }
    /** Source-compatible constructor used by legacy tests and APIs; rich fields receive explicit unknown/default values. */
    public Player(Vec3 position,Vec3 velocity,float yaw,float pitch,boolean onGround,String gamemode,Map<String,Integer> effects,OptionalInt awaitingTeleport,boolean uncertain){this(position,velocity,yaw,pitch,onGround,gamemode,effects,awaitingTeleport,uncertain,Optional.empty(),new Simulation.Attributes(0.1),Phase5Mechanics.Pose.STANDING,Environment.UNKNOWN,TickRange.unknown(),Provenance.UNKNOWN,uncertain?Set.of(UncertaintyReason.EXPLICIT_UNCERTAINTY):Set.of(),0);}
    /** Source-compatible richer constructor; jump-delay state defaults to zero. */
    public Player(Vec3 position,Vec3 velocity,float yaw,float pitch,boolean onGround,String gamemode,Map<String,Integer> effects,OptionalInt awaitingTeleport,boolean uncertain,Optional<Simulation.AdvancedInput> input,Simulation.Attributes attributes,Phase5Mechanics.Pose pose,Environment environment,TickRange clientTickRange,Provenance provenance,Set<UncertaintyReason> uncertaintyReasons){this(position,velocity,yaw,pitch,onGround,gamemode,effects,awaitingTeleport,uncertain,input,attributes,pose,environment,clientTickRange,provenance,uncertaintyReasons,0);}
  ...
  public static Player apply(Player state,NormalizedPacket event){
    ...
    if(packet instanceof ClientTickEnd)return state;
    if(packet instanceof FlightToggle)return state;
