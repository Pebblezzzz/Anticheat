...
public final class Simulation {
  ...
  public record Attributes(double movementSpeed,List<Phase5Mechanics.AttributeModifier> modifiers) implements Serializable { public static final Attributes DEFAULT=new Attributes(0.1,List.of()); public Attributes(double movementSpeed){this(movementSpeed,List.of());} public Attributes { if(!Double.isFinite(movementSpeed)||movementSpeed<0)throw new IllegalArgumentException("invalid movement speed");modifiers=List.copyOf(modifiers); } public double value(){return Phase5Mechanics.resolveAttribute(movementSpeed,modifiers);} }
