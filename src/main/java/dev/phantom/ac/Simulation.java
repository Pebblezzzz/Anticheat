...
public final class Simulation {
  ...
  public static final class Vanilla12111Physics implements Contracts.PhysicsEngine {
    ...
    private StepResult step(TickContext context,long tick,AdvancedInput input,Environment environment,
      ...
      if(environment==Environment.UNKNOWN) return new StepResult(tick,uncertain(s),false,priorPose,"movement environment is unknown");
      ...
      if(!s.gamemode().equals("survival") && !s.gamemode().equals("adventure")) return new StepResult(tick,uncertain(s),false,priorPose,"unsupported gamemode movement model");

      Phase5Mechanics.Pose requestedPose=Phase5Mechanics.nextPose(priorPose,env);
      Aabb start=Aabb.playerAt(s.position(),requestedPose);
      ...
      if(climbing) {
        if(input.forward()>0) velocity=new Vec3(velocity.x(),CLIMB_MAX_UP,velocity.z());
        else if(input.forward()<0) velocity=new Vec3(velocity.x(),CLIMB_MAX_DOWN,velocity.z());
