...
public final class Replay {
  private Replay() {}
  public record Frame(int index,Event event,Player before,Player after) {
    public Frame { if(index<0)throw new IllegalArgumentException("frame index must be non-negative");Objects.requireNonNull(event,"event");Objects.requireNonNull(before,"before");Objects.requireNonNull(after,"after"); }
  }
  public record Result(List<Frame> frames) {
    public Result { frames=List.copyOf(frames); }
    public List<Player> states(){return frames.stream().map(Frame::after).toList();}
    public Player finalState(){if(frames.isEmpty())throw new NoSuchElementException("empty replay has no final state");return frames.getLast().after();}
  }
  ...
  public static Result replay(Player start,Snapshot timeline){
    Objects.requireNonNull(start,"start");Objects.requireNonNull(timeline,"timeline");
    List<Frame> frames=new ArrayList<>();Player current=start;int index=0;
    for(Event event:timeline.events()){Player before=current;current=State.apply(current,event.packet());frames.add(new Frame(index++,event,before,current));}
  ...
  /** Reports the first event/state field at which two replay results differ. */
  public static Optional<Divergence> firstDivergence(Result expected,Result actual){
    ...
    for(int index=0;index<shared;index++){
      Frame left=expected.frames().get(index),right=actual.frames().get(index);
      if(!left.event().equals(right.event()))return Optional.of(new Divergence(index,left.event().serverTick(),left.event().packet().sequence(),"event",left.event(),right.event()));
      Optional<Divergence> state=stateDifference(index,left.event(),left.after(),right.after());if(state.isPresent())return state;
    }
    if(expected.frames().size()!=actual.frames().size()){
      Frame frame=expected.frames().size()>shared?expected.frames().get(shared):actual.frames().get(shared);
      return Optional.of(new Divergence(shared,frame.event().serverTick(),frame.event().packet().sequence(),"replay length",expected.frames().size(),actual.frames().size()));
    }
