 ...
 *
 * <p>Unlike the legacy comparator, this retains the previous vanilla row,
 * previous simulated frame, input timing, environment/effect context and
 * collision/correction observations so the next physics fix has a concrete
 * causal starting point.</p>
 */
public final class Phase5VanillaDetailedComparison {
    ...
    public record Diagnostic(
            ...
            String actual,
            double delta,
            StateContext previousVanilla,
            StateContext currentVanilla,
            String simulatedBefore,
            String simulatedAfter,
            String explanation
    ) implements Serializable {}

    public static Optional<Diagnostic> firstDivergence(
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!expected.missingFields().contains("forward") && !expected.missingFields().contains("strafe") && !expected.missingFields().contains("jump")) {
                ...
                if (!expectedInput.equals(actualInput)) return Optional.of(diag(tick, Cause.INPUT_TIMING, "input", expectedInput, actualInput, 0, previousVanilla, currentVanilla, previousActual, actual, "pre-tick control input differs from the replayed input"));
            }
            ...
            if (!expected.missingFields().contains("pose") && !expected.pose().equalsIgnoreCase(actual.state().gamemode())) {
                // Pose is intentionally not mapped into Simulation.Player. Keep it as context rather than fabricating a state comparison.
            }

            if (!expected.missingFields().contains("velocity_packet") && Boolean.parseBoolean(expected.velocityPacket())) {
                return comparePacketContext(expected, previousExpected, previousVanilla, currentVanilla, previousActual, actual);
            }
            if (!expected.missingFields().contains("correction_id") && Long.parseLong(expected.correctionId()) >= 0) {
                return Optional.empty(); // correction is represented as a state barrier; the state reducer decides whether it is replayable.
            }
        }
        ...
        if (vanilla.rows().size() != simulated.frames().size()) {
            ...
            return Optional.of(new Diagnostic(tick, Cause.TICK_ALIGNMENT, "trace_length", Integer.toString(vanilla.rows().size()), Integer.toString(simulated.frames().size()), simulated.frames().size() - vanilla.rows().size(), null, index < vanilla.rows().size() ? context(vanilla.rows().get(index)) : null, null, index < simulated.frames().size() ? simulated.frames().get(index) : null, "trace length differs"));
        }
        return Optional.empty();
    }

    private static Optional<Diagnostic> comparePacketContext(Phase5VanillaTrace.Row expected, Phase5VanillaTrace.Row previousExpected, StateContext previousVanilla, StateContext currentVanilla, Simulation.Frame previousActual, Simulation.Frame actual) {
        if (!expected.missingFields().contains("knockback_x") || !expected.missingFields().contains("knockback_y") || !expected.missingFields().contains("knockback_z")) {
            double packetMagnitude = Math.sqrt(square(Double.parseDouble(expected.knockbackX())) + square(Double.parseDouble(expected.knockbackY())) + square(Double.parseDouble(expected.knockbackZ())));
            if (packetMagnitude > 0.0) return Optional.of(diag(parseLong(expected.tick()), Cause.KNOCKBACK_OR_VELOCITY_PACKET, "velocity_packet", expected.knockbackX() + "," + expected.knockbackY() + "," + expected.knockbackZ(), actual.state().velocity().x() + "," + actual.state().velocity().y() + "," + actual.state().velocity().z(), 0, previousVanilla, currentVanilla, previousActual, actual, "authoritative velocity packet observed; replay must model the packet transition before physics"));
        }
        return Optional.empty();
    }
    ...
    private static Diagnostic diag(long tick, Cause cause, String field, String expected, String actual, double delta, StateContext previousVanilla, StateContext currentVanilla, Simulation.Frame previousActual, Simulation.Frame actualFrame, String explanation) {
        return new Diagnostic(tick, cause, field, expected, actual, delta, previousVanilla, currentVanilla, describe(previousActual), describe(actualFrame), explanation);
    }

    private static String describe(Simulation.Frame frame) {
        if (frame == null) return "<none>";
        return "tick=" + frame.tick() + " pos=" + frame.state().position() + " vel=" + frame.state().velocity() + " ground=" + frame.state().onGround() + " input=" + frame.input();
    }
    ...
    private static StateContext context(Phase5VanillaTrace.Row row) {
        return new StateContext(
                ...
                row.forward() + "," + row.strafe() + "," + row.jump(),
                row.pose(), row.fluid(),
                "speed=" + row.speedAmp() + ",slow=" + row.slownessAmp() + ",jump=" + row.jumpBoostAmp() + ",levitation=" + row.levitation() + ",slow_falling=" + row.slowFalling(),
                row.modifiers(),
                "collision=" + row.collision() + ",x=" + row.collisionX() + ",y=" + row.collisionY() + ",z=" + row.collisionZ(),
                "id=" + row.correctionId() + ",pending=" + row.correctionPending(),
                row.worldIdentity());
    }
