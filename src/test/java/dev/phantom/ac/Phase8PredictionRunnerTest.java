...
class Phase8PredictionRunnerTest {
  ...
  @Test
  void postMovementAuthoritySnapshotCannotClearLivePhysicsFrontier() {
    Phase8PredictionRunner runner = new Phase8PredictionRunner(4096);
    WorldSnapshot world = floorWorld();

    Player start = new Player(
        new Maths.Vec3(.5, 70.0, .5),
        new Maths.Vec3(0.0, 0.0, .2),
        0f, 0f, false, "survival", Map.of(),
        OptionalInt.empty(), false, Optional.empty(),
        Simulation.Attributes.DEFAULT, Pose.STANDING, State.Environment.DRY,
        State.TickRange.exact(0), State.Provenance.UNKNOWN, Set.of());
    ...
    PlayerContext startAuthority = new PlayerContext(
        "survival", start.attributes(), Map.of(), Pose.STANDING, environment,
        start.position(), start.velocity(), false, false, false, List.of());
    ...
    PlayerContext postMovementAuthority = new PlayerContext(
        "survival", start.attributes(), Map.of(), Pose.STANDING, environment,
    ...
    var report = runner.process(
        "post-movement-authority-frontier",
        List.of(
            new RawPacket(
                1L, 10L, startAuthority,
                Packets.CaptureProvenance.fromAdapter(
                    "test-authority-start", startAuthority, 0L, 0L)),
            new RawPacket(
                2L, 20L, firstMovement,
                Packets.CaptureProvenance.fromAdapter(
                    "test-movement", firstMovement, 0L, 1L)),
            new RawPacket(
                3L, 30L, postMovementAuthority,
                Packets.CaptureProvenance.fromAdapter(
                    "test-authority-post", postMovementAuthority, 1L, 2L)),
            new RawPacket(
                4L, 40L, secondMovement,
                Packets.CaptureProvenance.fromAdapter(
                    "test-movement", secondMovement, 1L, 2L))),
        world,
        start,
        0L);
    ...
    assertTrue(report.candidateFrontierRetained(), report.toString());
    assertTrue(report.frames().getLast().trace().stream()
        .anyMatch(line -> line.startsWith("PREDICT_FORWARD")
            || line.startsWith("FRONTIER_COMMITTED")
            || line.contains("FRONTIER_RETAINED")),
        report.frames().getLast().trace().toString());
    assertTrue(report.frames().getLast().trace().stream()
        .noneMatch(line -> line.contains("AUTHORITATIVE_ZERO_DELTA_WITNESS")),
        report.frames().getLast().trace().toString());
  }
