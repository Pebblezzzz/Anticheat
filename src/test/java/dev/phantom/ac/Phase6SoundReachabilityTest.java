...
class Phase6SoundReachabilityTest {
  private static final WorldSnapshot FLOOR=WorldSnapshot.builder(Contracts.TARGET_VERSION).loadChunk(0,0)
  ...
  private static Phase6Reachability.Context start(){
    Player p=Player.initial(new Maths.Vec3(0.5,65.0,0.5));
    return new Phase6Reachability.Context(0,p,Simulation.Environment.DRY,Attributes.DEFAULT,MovementEffects.NONE,Pose.STANDING,MovementEnvironment.dry(true,false,false),false);
  }
  ...
  @Test void exactStateMergeIsSafeAndTracksProvenance(){
    SearchResult r=ENGINE.search(start(),List.of(InputConstraint.exact(new AdvancedInput(0,0,false,false,false))),
        t->List.of(new WorldBranch("a",FLOOR,true,"same world"),new WorldBranch("b",FLOOR,true,"same world duplicate")),
        t->List.of(new Phase6Reachability.None()),1000);
    assertEquals(Verdict.POSSIBLE,r.verdict());assertTrue(r.mergedStates()>0);assertTrue(r.provenanceMerges()>0);
    Candidate c=r.candidates().iterator().next();assertTrue(c.provenance().mergedPathCount()>1);assertFalse(c.provenance().mergedParentIds().isEmpty());
  }
  @Test void candidateBudgetOverflowNeverExposesASampledSubset(){SearchResult r=search(InputConstraint.any());assertEquals(72,r.candidates().size());SearchResult limited=ENGINE.search(start(),List.of(InputConstraint.any()),t->List.of(new WorldBranch("floor",FLOOR,true,"known loaded floor")),t->List.of(new Phase6Reachability.None()),10);assertEquals(Verdict.UNCERTAIN,limited.verdict());assertTrue(limited.candidates().isEmpty());}
  ...
  @Test void replayArtifactIsSelfContainedAndDeterministic(){SearchResult r=search(InputConstraint.exact(new AdvancedInput(0,0,false,false,false)));Phase6Replay replay=Phase6Replay.of(start(),List.of(InputConstraint.exact(new AdvancedInput(0,0,false,false,false))),0,0,false,Map.of(0,List.of(new WorldBranch("floor",FLOOR,true,"known floor"))),Map.of(0,List.of(new Phase6Reachability.None())),r);assertEquals(Phase6Replay.SCHEMA_VERSION,replay.schemaVersion());assertTrue(replay.canonicalText().startsWith(Phase6Replay.SCHEMA_VERSION));}
