...
class Phase6SoundReachabilityTest {
  ...
  @Test void fullCandidateContextKeepsSprintAndSneakAsDistinctFutureState(){SearchResult r=search(InputConstraint.any());assertEquals(Verdict.UNCERTAIN,r.verdict());assertEquals(72,r.candidates().size());assertFalse(r.metrics().exhaustive());assertTrue(r.metrics().generatedCandidates()>=72);}
  @Test void exactStateMergeIsSafeAndTracksProvenance(){SearchResult r=ENGINE.searchWithTransitionBranches(List.of(start()),List.of(InputConstraint.exact(new AdvancedInput(0,0,false,false,false))),t->List.of(new WorldBranch("floor",FLOOR,true,"same world")),t->List.of(new Phase6Reachability.ExternalPath("a",List.of(),"same state path A"),new Phase6Reachability.ExternalPath("b",List.of(new Phase6Reachability.None()),"same state path B")),new Phase6Reachability.SearchConfig(1000,8,100,1000L,null,"merge-test"));assertEquals(Verdict.POSSIBLE,r.verdict());assertTrue(r.mergedStates()>0);assertTrue(r.provenanceMerges()>0);Candidate c=r.candidates().iterator().next();assertEquals(2,c.provenance().mergedPathCount());assertFalse(c.provenance().assumptions().isEmpty());}
  @Test void candidateBudgetOverflowRetainsOnlyADeterministicSafeSubset(){SearchResult limited=ENGINE.search(start(),List.of(InputConstraint.any()),t->List.of(new WorldBranch("floor",FLOOR,true,"known loaded floor")),t->List.of(new Phase6Reachability.None()),10);assertEquals(Verdict.UNCERTAIN,limited.verdict());assertEquals(10,limited.candidates().size());assertTrue(limited.metrics().prunedCandidates()>0);assertTrue(limited.metrics().budgetReached());}
  @Test void unknownWorldCoverageIsUncertain(){WorldSnapshot unknown=WorldSnapshot.emptyOverworld12111();SearchResult r=ENGINE.search(start(),List.of(InputConstraint.exact(new AdvancedInput(0,0,false,false,false))),t->List.of(new WorldBranch("unknown",unknown,true,"unloaded world")),t->List.of(new Phase6Reachability.None()),100);assertEquals(Verdict.UNCERTAIN,r.verdict());assertTrue(r.candidates().isEmpty());assertTrue(r.eliminations().stream().anyMatch(e->e.worldKnowledge()==Phase6Reachability.WorldKnowledge.UNLOADED));}
  @Test void nonExhaustiveWorldHypothesisIsUncertain(){SearchResult r=ENGINE.search(start(),List.of(InputConstraint.exact(new AdvancedInput(0,0,false,false,false))),t->List.of(new WorldBranch("partial",FLOOR,false,"only one of several possible world states")),t->List.of(new Phase6Reachability.None()),100);assertEquals(Verdict.UNCERTAIN,r.verdict());assertFalse(r.candidates().isEmpty());assertTrue(r.candidates().iterator().next().context().uncertainty().contains(Phase6Reachability.UncertainDimension.WORLD));}
  ...
  @Test void timingWindowEvaluatesEveryOffsetAndRetainsExhaustiveCandidates(){WorldSnapshot unknown=WorldSnapshot.emptyOverworld12111();var r=ENGINE.searchWithinTimingWindow(start(),0,1,false,List.of(InputConstraint.exact(new AdvancedInput(0,0,false,false,false))),t->t==0?List.of(new WorldBranch("unknown",unknown,true,"unloaded offset")):List.of(new WorldBranch("floor",FLOOR,true,"known offset")),t->List.of(new Phase6Reachability.None()),100);assertEquals(Verdict.UNCERTAIN,r.verdict());assertEquals(2,r.evaluatedOffsets());assertEquals(0,r.skippedOffsets());assertTrue(r.byFirstTick().containsKey(0L));assertTrue(r.byFirstTick().containsKey(1L));assertFalse(r.candidates().isEmpty());assertTrue(r.reasons().stream().anyMatch(s->s.toLowerCase(Locale.ROOT).contains("uncertain")));}
