...
class Phase6SoundReachabilityTest {
  private static final WorldSnapshot FLOOR=WorldSnapshot.builder(Contracts.TARGET_VERSION).loadChunk(0,0).setBlock(0,64,0,dev.phantom.ac.world.v12111.BlockCatalogue12111.decode("minecraft:stone",Map.of())).build();
  ...
  @Test void candidateBudgetOverflowNeverExposesASampledSubset(){SearchResult r=search(InputConstraint.any());assertEquals(Verdict.POSSIBLE,r.verdict());assertEquals(72,r.candidates().size());SearchResult limited=ENGINE.search(start(),List.of(InputConstraint.any()),t->List.of(new WorldBranch("floor",FLOOR,true,"known loaded floor")),t->List.of(new Phase6Reachability.None()),10);assertEquals(Verdict.UNCERTAIN,limited.verdict());assertTrue(limited.candidates().isEmpty());}
