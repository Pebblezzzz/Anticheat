...
class Phase8GrimKnownInputLifecycleTest {
  ...
  @Test
  void causallyPriorJumpInputIsRetainedAtTheFinalMovementBoundary() {
    WorldSnapshot world = floorWorld();
    Vanilla12111RichPhysics physics = new Vanilla12111RichPhysics();
    Player start = new Player(
        new Maths.Vec3(0.5, 70.0, 0.5),
        Maths.Vec3.ZERO,
        0f,
        0f,
        true,
    ...
    Phase5MovementAuthority.StepResult expectedStep =
        new Phase5MovementAuthority().simulate(
            new Phase5MovementAuthority.SimulationContext(
                0L,
                start,
                ...
                false,
                EntityCollisions.of(List.of())));
    Player expected = expectedStep.state();

    MovementEnvironment physicalGround =
        MovementEnvironment.dry(true, false, false);
    PlayerContext authority = new PlayerContext(
        "survival",
        start.attributes(),
    ...
    var report = runner.process(
        ...
        List.of(
            ...
            new RawPacket(3, 30L, new Move(
                expected.position(), 0f, 0f, expected.onGround(), 1L))),
        world,
        start,
        0L);
    ...
    String finalInputs = report.frames().getLast().trace().stream()
        .filter(line -> line.contains("SIM_INPUT_OPTIONS tick=0"))
        .findFirst()
        .orElseThrow();
    assertTrue(finalInputs.contains("jump=Optional[true]"), finalInputs);
  }
